...
| 갱신자 | 개발 (아키텍트) → QA (리드) |
| 전체 진행률 | **36 / 44 작업 완료 (82%)** · MVP 범위는 41작업 (Phase 5 제외) · B-11 코드 완료 · 실검증 대기 |
...
**마지막 커밋:** `6366e70` feat(B-11) 주간 마감의 최종 캘린더 동기화 연결 · 2026-08-20
→ **이 세션에서 T-03 통합 테스트도 커밋**
...
### ✅ 이 세션에서 추가로 한 것 (2026-08-20 오후) — B-11 마감 연결 · T-03

**1. `closing.ts` 의 `performFinalCalendarSync` → `syncCalendarWeek` 연결** (N-032 의 빈 자리를 채웠다)
...
- ⭐ **`pnpm test:tz`** — UTC · Asia/Seoul · America/New_York **세 존으로 3회** 돌린다 (§6 #39-b).
  `vitest.config.ts` 가 TZ 를 못 박지 않고 밖에서 받는다(기본 UTC). 러너는 `scripts/test-timezones.mjs`
  — 셸이 아니라 node 인 이유는 `TZ=... pnpm test` 가 Windows PowerShell 에서 안 먹기 때문
- ⚠️ **실 DB 를 쓰지 않는다** (**N-036**). `../prisma` 와 구글 `fetch` 를 모듈 경계에서 갈아 끼운다.
  그래서 **SQL 자체(UNIQUE·`skipDuplicates`·`groupBy`)는 검증되지 않는다** — 스키마를 바꾸면 실 DB 로 다시 태울 것
- 🐛 처음엔 테스트 안에 `expect(new Date().getTimezoneOffset()).toBe(0)` 을 넣었다가 `test:tz` 에서
  그 두 개만 깨졌다. 지켜야 할 성질은 *"UTC 에서 돈다"* 가 아니라 **"존을 바꿔도 답이 같다"** 였다
...
| 항목 | 상태 |
|---|---|
| **배포본이 로컬보다 뒤처져 있다** | 마지막 CLI 배포 이후 U-03·B-06·B-07·U-04·**B-08·B-09·U-06·U-05·F-01·F-02·B-11·T-03** 이 안 올라갔다 |
